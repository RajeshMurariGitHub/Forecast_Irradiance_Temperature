...
class FeatureEngineer:
    ...
    def add_temporal_features(self) -> 'FeatureEngineer':
        ...
        logger.info(f"Added {len([f for f in self.features_added if 'hour' in f or 'month' in f or 'day' in f])} temporal features")
    ...
    def add_lag_features(self, columns: List[str], lags: List[int]) -> 'FeatureEngineer':
        ...
        logger.info(f"Added {len([f for f in self.features_added if 'lag' in f])} lag features")
    ...
    def add_rolling_features(self, columns: List[str], windows: List[int], 
        ...
        logger.info(f"Added {len([f for f in self.features_added if 'rolling' in f])} rolling features")
    ...
    def add_difference_features(self, columns: List[str], diffs: List[int]) -> 'FeatureEngineer':
        ...
        """
        logger.info(f"Adding difference features")
    ...
    def add_interaction_features(self, feature_pairs: Optional[List[tuple]] = None) -> 'FeatureEngineer':
        """
        ...
        logger.info(f"Adding {len(feature_pairs)} interaction features")
    ...
    def add_ratio_features(self, numerator: str, denominator: str, name: Optional[str] = None) -> 'FeatureEngineer':
        """
        ...
        if numerator not in self.df.columns or denominator not in self.df.columns:
            logger.warning(f"Columns not found, skipping {name}")
            return self
    ...
    def handle_missing_values(self, method: str = 'forward_fill', limit: int = 3) -> 'FeatureEngineer':
        """
        ...
        logger.info(f"Reduced missing values from {initial_na:,} to {final_na:,}")
